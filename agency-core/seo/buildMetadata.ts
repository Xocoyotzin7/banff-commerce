...
import type { SeoPageMetadataInput, SeoConfig, SeoLocale, SeoRobots } from "@banff/agency-core/seo/types"

export function buildLayoutMetadata(config: SeoConfig = seoConfig): Metadata {
  // Layout-level metadata stays reusable; page modules can extend or override it.
  return {
...
export function buildPageMetadata(input: SeoPageMetadataInput, config: SeoConfig = seoConfig): Metadata {
  // Page metadata is assembled from route data only. Do not mix view copy with SEO wiring.
