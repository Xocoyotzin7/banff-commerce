  ...
  try {
    if (isAdminDemoMode()) {
      return NextResponse.json({
        success: true,
        data: getDemoTravelInsightsPayload(range, month),
    ...
    const data = await getAdminTravelInsightsPayload(database.db, range, month)
    return NextResponse.json({
      success: true,
      data,
    }, { headers: { "Cache-Control": "no-store" } })
    ...
    const message = error instanceof Error ? error.message : "Unable to load travel insights"
