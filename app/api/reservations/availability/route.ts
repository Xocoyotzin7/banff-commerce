...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!dateParam) {
      return NextResponse.json({ success: false, message: "Debes indicar una fecha (YYYY-MM-DD)." }, { status: 400 })
    }
    ...
    const rows = await database
      .select({ reservationTime: reservations.reservationTime })
      .from(reservations)
      .where(and(eq(reservations.branchId, branchId), eq(reservations.reservationDate, reservationDate)))
